...
function indent(str, additionalIndentation = '  ') {
  return str.split('\n').map(s => s ? additionalIndentation + s : '').join('\n');
}
...
function getProjectSources(options, config) {
  return __awaiter(this, void 0, void 0, function* () {
    ...
    if (options.input !== undefined && options.input.length > 0) {
      ...
      const absPaths = yield globby(options.input, {
        root: process.cwd()
      });
      return absPaths.map(p => path.relative(config.root, p));
    }

    const candidateFiles = yield globby(config.sources, {
      root: config.root
    });
...
exports.getProjectSources = getProjectSources; //# sourceMappingURL=util.js.map