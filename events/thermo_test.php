...
Scheduler::register_task("Lights On", "pi1", new WeeklyTimer("*", [16], [0],[0]), function(){
	get_item("TV Light")->set_value(Session::$INTERNAL, new Value(1));
	get_item("Table Lamp")->set_value(ession::$INTERNAL, new Value(1));
	get_item("Nick Bed Lights")->set_value(ession::$INTERNAL, new Value(1));
});
Scheduler::register_task("Living Lights off", "pi1", new WeeklyTimer("*", [23], [0],[0]), function(){
	get_item("TV Light")->set_value(Session::$INTERNAL, new Value(0));
	get_item("Table Lamp")->set_value(Session::$INTERNAL, new Value(0));
});
