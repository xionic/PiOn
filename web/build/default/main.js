...
function get_module_html(name, item_name) {
  ...
  let obj = {
    html: '<module-' + name + " class='itemmodule' id='module_container_" + module_id_counter + "' item_name='" + item_name + "'></module-" + name + ">",
    ...
    $(Object.keys(sitemap)).each(function (k, room) {
      ...
      $(sitemap[room]).each(function (key, item) {
        ...
        try {
          ...
          let rest_message = new RestMessage(RestMessage.REQ, RestMessage.REST_CONTEXT_ITEM, "client", null, null, item_message); //websocket.send(rest_message.to_json());
          ...
          if (e instanceof InvalidModuleExeption) return; //jquery .each equiv of continue
          else throw e;
        }
      });
